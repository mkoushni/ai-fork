...
| `headers.effective_model` | string | no | Header name for the effective (post-rewrite) model value. Must not be a hop-by-hop, framing, Host, credential, or unrelated internal `x-praxis-*` header. Must differ from `original_model`. |
| `headers.original_model` | string | no | Header name for the original (pre-rewrite) model value. Must not be a hop-by-hop, framing, Host, credential, or unrelated internal `x-praxis-*` header. Must differ from `effective_model`. |
