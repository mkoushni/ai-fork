...
fn body_access_is_read_only() {
    ...
    assert_eq!(
        ...
        praxis_filter::body::BodyAccess::ReadOnly,
...
fn task_routing_rejects_zero_max_response_body_bytes() {
    ...
    let err = A2aFilter::from_config(&yaml).err().expect("should fail");
    assert!(
        err.to_string().contains("'max_body_bytes' must be greater than 0"),
...
fn seed_response_capture(ctx: &mut praxis_filter::HttpFilterContext<'_>) {
    ctx.filter_metadata
...
fn make_a2a_request(extra_headers: &[(&str, &str)]) -> praxis_filter::Request {
...
fn seed_sse_capture(ctx: &mut praxis_filter::HttpFilterContext<'_>) {
    ctx.filter_metadata
...
fn assert_sse_capture_cleared(ctx: &praxis_filter::HttpFilterContext<'_>) {
    assert!(
